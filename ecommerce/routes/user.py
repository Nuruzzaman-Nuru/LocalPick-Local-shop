...
try:
    ...
    def process_negotiation(negotiation):
        """Fallback negotiation processor used when AI module is unavailable.

        Returns a tuple: (decision, counter_offer, message)
        decision is one of: 'accept', 'reject', 'counter_offer'
        """
        try:
            offered = float(getattr(negotiation, 'offered_price', 0) or 0)
        except Exception:
            offered = None

        product = getattr(negotiation, 'product', None)
